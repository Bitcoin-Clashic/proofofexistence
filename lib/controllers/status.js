...
console.log( "Doc Address: " + docAddress )

  if (docAddress) {
    const docproof = await store.getDocproof(docAddress)
console.log( "Doc docproof: " + JSON.stringify(docproof) )
    // Wait for payment
...
async function statusInBlockchain (hash) {
  ...
  const urlr = 'https://insight.clashic.cash/api/tx/'
  console.log(hash);
...
console.log(JSON.stringify(doc.txid));
